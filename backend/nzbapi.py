...
def indexer_search(q: str, cfg: ConfigManager):
    ...
    if "error" in response.text: raise HTTPException(status_code=403, detail=response.text)
    data = response.json()
    return data
...
def download(nzb : bytes, cfg: ConfigManager, nzbname: str = str(uuid4())):
    ...
    if a.status_code != 200: raise HTTPException(status_code=a.status_code, detail=a.text)